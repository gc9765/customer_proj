    ...
    switch(event_id&0xffff){
        ...
        case SYSEVT_WIFI_CONNECTTED:
            ...
            sys_status.wifi_connected = 1;
            {
                ...
                // os_printf("###############################\r\n");
                // os_printf("## bssid = %d,channel =%d\r\n",bssid,channel);
                // os_printf("## bss_data.proto = %d,.group_cipher = %d,.pairwise_cipher = %d,.key_mgmt= =%d\r\n",\
                //                     bss_data.proto, bss_data.group_cipher, bss_data.pairwise_cipher, bss_data.key_mgmt);
                // os_printf("###############################\r\n");
		...
		case SYSEVT_WIFI_STA_DISCONNECT:
			aid_to_mac((uint16)data,mac);
			user_sta_del((char *)mac);
            // sta_connect_ap_flag = 0;           
			break;
		case SYSEVT_WIFI_STA_CONNECTTED:
			aid_to_mac((uint16)data,mac);
			user_sta_add((char *)mac);
            // os_printf("**** sta scan down....... ***\r\n");
            // os_printf("**** sta scan down....... ***\r\n");
            // sta_connect_ap_flag = 1;           
			break;			
...
int main(void)
{
    ...
    vcam = vcam_en();
    user_io_preconfig();
	gpio_set_mode(PA_8, GPIO_PULL_NONE, GPIO_PULL_LEVEL_NONE);
	gpio_set_mode(PA_9, GPIO_PULL_NONE, GPIO_PULL_LEVEL_NONE);
	gpio_set_mode(PA_10, GPIO_PULL_NONE, GPIO_PULL_LEVEL_NONE);
    gpio_iomap_output(PA_8,GPIO_IOMAP_OUTPUT); 
	gpio_set_val(PA_8,0);
...
//	gpio_set_val(PA_5,1);	
    ...
    if(system_is_wifi_test_mode()) {
        ...
        const uint8 temp_en_str[] = "at+pcf_chn=114688\n";
        atcmd_recv((void *)temp_en_str, sizeof(temp_en_str));
