...
int bbm_start_record(void)
{
    ...
    if(avi_s)
    {
        ...
        os_printf("fps:%d\tfrq:%d\n",avi_s->fps,avi_s->frq);
        OS_TASK_INIT("at_avi", &avi_s->task, at_save_avi_thread, (uint32)avi_s, OS_TASK_PRIORITY_NORMAL, 1024*4);  
    }
    return 1;
}

void bbm_stop_record(void)
{
        if(avi_s)
        {
            avi_s->running = 0;
...
uint8_t is_bbmavi_running(void)
{
	return avi_s->running?1:0;
}
