...
mp3_malloc g_mp3_malloc = NULL;
mp3_free   g_mp3_free   = NULL;
mp3_calloc g_mp3_calloc = NULL;
...
{
    //需要初始化空间申请、释放函数
    if(!g_mp3_malloc || !g_mp3_free || !g_mp3_calloc )
    {
      return -2;
    }
    int result;
...
# endif
    ...
    decoder->sync = g_mp3_calloc(sizeof(*decoder->sync),1);
    if (decoder->sync == 0)
        return -1;

    result = run(decoder);

    g_mp3_free(decoder->sync);
    decoder->sync = 0;
