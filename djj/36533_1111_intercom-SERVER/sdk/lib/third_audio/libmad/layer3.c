			  ...
			  struct sideinfo *si, unsigned int nch)
{
    ...
    for (gr = 0; gr < ngr; ++gr) {
        ...
        mad_fixed_t xr[1][576]; 
#else
        mad_fixed_t xr[2][576];
#endif       
        unsigned int ch;
        enum mad_error error;
        mad_fixed_t *xr_p = NULL;
        ...
        for (ch = 0; ch < nch; ++ch) {
            ...
            error = III_huffdecode(ptr, xr_p, channel, sfbwidth[ch], part2_length);
            if (error)
                return error;
#if !FORCE_MONO_CHANNEL
            if(ch == (nch-1)) {
...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
    ...
    if (stream->main_data == 0) {
        stream->main_data = g_mp3_malloc(MAD_BUFFER_MDLEN);
        if (stream->main_data == 0) {
    ...
    if (frame->overlap == 0) {
    #if FORCE_MONO_CHANNEL
        frame->overlap = g_mp3_calloc(1 * 32 * 18, sizeof(mad_fixed_t));
    #else
        frame->overlap = g_mp3_calloc(2 * 32 * 18, sizeof(mad_fixed_t));
    #endif
        if (frame->overlap == 0) {
            stream->error = MAD_ERROR_NOMEM;
            return -1;
